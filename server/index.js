...
const ChatM = require('./models/Chat'); // Ensure your Chat model uses username
...
// Chat route with history linking
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    res.end(aiResponse); // Send the complete response
...
app.post('/api/file', upload.single('files'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      // Generate content based on the prompt and file data
...
app.post('/api/audio', upload.single('files'), async (req, res) => {
  ...
  try {
    if (req.file) {
      ...
      const promptToUse = prompt || 'reply to this audio'; // Use provided prompt or default
