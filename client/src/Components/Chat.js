...
const Chat = ({ username }) => {
  ...
  // Fetch chat history upon component mounting
  useEffect(() => {
  ...
  const extractCodeBlock = (response) => {
    const code = response.match(/```(.*?)```/s); // Matches code enclosed in triple backticks
  ...
  const handleQuestionSubmit = async () => {
    ...
    try {
      ...
      // Append AI and user responses to chat history
      setHistory((prev) => [
        ...prev,
  ...
  // Auto-scroll to the bottom of the chat on history update
  useEffect(() => {
